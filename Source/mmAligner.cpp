...
* History : 
* 
* February, 2014 : implemented maxTag feature for m2m alignment with multi-character suffixes
*                  while most characters can align well to themselves,
*		   an abstract morphological tag often needs to align to many more than 1 or
*		   2 characters.  Hence, maxTag allows larger alignments, including x to x size
*		   alignments if the character is indicated as a tag by a special character.
*
*
*
*
...
int main(int argc, char** argv)
{
	...
	try{
		...
		ValueArg<int> maxTag("", "maxTag", "Allow separate y maximum for specialty tags (default maxY)", false, maxY.getValue(), "int", cmd);

		ValueArg<string> tagIndicator("", "tagInd", "string attached to tags for tag alignment (default *)", false, "*", "string", cmd);
		SwitchArg copy("", "copy", "Learn probabilities that take copying into account, (default false)", cmd, false);

		ValueArg<double> copyBias("", "copyBias", "Bias towards copying before stable counts are learned(default ~1.0)", false, 0.999999, "double", cmd);
		...
		myParam.maxTag = maxTag.getValue();
		myParam.tagIndicator = tagIndicator.getValue();
		myParam.copy = copy.getValue();
		myParam.copyBias = copyBias.getValue();
		if(myParam.maxTag == 0)
		{
			myParam.maxTag = maxY.getValue();
		}
		...
		{
			myParam.prefixProcess = "m-mAlign." + stringify(myParam.maxX) + "-" + stringify(myParam.maxY) + "-" + stringify(myParam.maxTag);
			if (myParam.delX)
			{
		...
		{
			// training aligner //
			myAligner.training(myParam, copyProb);
		...
		myAligner.createAlignments(myParam, copyProb);
		printTimeStamp(myParam.startT);		
	}
