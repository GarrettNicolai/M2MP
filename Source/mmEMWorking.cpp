...
vector_2Double mmEM::backwardEval(param myParam, vector_str x, vector_str y)
{
	...
	for (int xl = x.size() ; xl >= 0; xl--)
	{
		...
		{
			if(xl + currentInsertion >= x.size())
			{
				//cout << xl << "\t" << currentInsertion << "\tBOUNDARY!" << endl;
				beta[xl][xl] = 1.0;
				continue;
			}
			if(xl + pastInsertion >= x.size())
			{
				//cout << xl << "\t" << currentInsertion << "\tBOUNDARY2!" << endl;
				beta[xl][xl] = 1.0;
				continue;
			}
				...
				for (int i = 1; (i <= myParam.maxX) && (xl-i-currentInsertion >= 0); i++)
				{
					...
					for (int j = 1; (j <= maxRight) && (yl-j-currentInsertion >= 0); j++)
					{
						...
						else
						{
							...
							//cout << "ALPHA\t" << ssX << "\t" << ssY << "\t" << alpha[xl-i-currentInsertion][yl-j-currentInsertion] << endl;
							//cout << "PROBS\t" << probs[ssX][ssY] << endl;
		...
		for (int yl = 0; yl <= y.size(); yl++)
		{
			...
			if ( (yl > 0) && (xl > 0) )
			{
				for (int i = 1; (i <= myParam.maxX) && (xl-i-currentInsertion >= 0); i++)
				{
					...
					for (int j = 1; (j <= maxRight) && (yl-j-currentInsertion >= 0); j++)
					{
						if(xl-pastInsertion == 0 && xl == yl)
						{
							cout << "START" << endl;
							string ssX = join(x, 0, xl);
							string ssY = join(y, 0, yl);
	
							long double updateCount;
							updateCount = (alpha[xl][yl] * probs[ssX][ssY] * beta[0][0]) / alpha_x_y;
						...
						long double updateCount;
						updateCount = (alpha[xl-i][yl-i] * probs[ssX][ssY] * beta[xl][yl]) / alpha_x_y;
						...
						for(int insIndex = 1; insIndex <= pastInsertion; insIndex++)
						{
							...
							longSSX = join(x, xl-i-currentInsertion-insIndex, i+currentInsertion+insIndex);
							longSSY = join(y, yl-j-currentInsertion-insIndex, j+currentInsertion+insIndex);
							updateCount = (alpha[xl-i-currentInsertion][yl-i-currentInsertion] * probs[longSSX][longSSY] * beta[xl-insIndex+i][yl-insIndex+j]) / alpha_x_y;
						        if(longSSX.compare("_#_") == 0)
							{
								cout << "BLAH:\t" << longSSX << "\t" << longSSY << "\t" << probs[longSSX][longSSY] << endl;
							}	
							if(updateCount != 0)
							{
				...
				for (int i = 1; (i <= myParam.maxX) && (xl-i-currentInsertion >=0); i++)
				{
					...
					for (int j = 1; (j <= maxRight) && (yl-j-currentInsertion >=0); j++)
					{
						...
						if(xl - pastInsertion == 0 && xl == yl)
						{
							...
							string ssX = join(x, 0 , xl);
							string ssY = join(y, 0 , yl);
							long double score = (long double) logl(probs[ssX][ssY]) * max(xl,yl);
							qtmp.backX = xl;
							qtmp.backY = yl;
							for (int rindex = 0; rindex < Q[0][0].size(); rindex++)
							{
				
								qtmp.score = score + Q[0][0][rindex].score;
								qtmp.backR = rindex;
								Q[xl][yl].push_back(qtmp);
							}
